...
function ClientsKycForm2({}) {
  ...
  useEffect(() => {
    function handleError() {
      let pictureText = "Client photo is";
    ...
    uploadTask.on(
      ...
      (snapshot) => {
        ...
        if (urlType === "jobPhotos") {
          setBrandProgress(Math.round(brandProgress));
        }
        if (urlType === "picture") {
          setProgress(Math.round(progress));
        }
        if (urlType === "coverPicture") {
          setCoverProgress(Math.round(coverProgress));
        }
  ...
  useEffect(() => {
    ...
    sendCoverPicture();
  }, [setInputs, error, coverPicture, jobPhoto, jobPhotos, picture]);
